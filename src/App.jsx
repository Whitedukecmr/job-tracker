...
export default function App() {
  ...
  const addRow = () => { saveRows([...rows, { ...form }]); setForm(newRow()); setShowAddForm(false); };
  ...
  const saveLabel = { idle:"", saving:"⏳ Sauvegarde...", saved:"✓ Sauvegardé sur GitHub", error:"⚠ Erreur" };
    ...
    <div style={{ fontFamily:"Arial,sans-serif", background:"#F0F4FA", minHeight:"100vh", padding:"28px 20px" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          <span style={{ fontSize:11, color:saveColor[saveStatus], fontWeight:600, minWidth:160 }}>{saveLabel[saveStatus]}</span>
      ...
      <div style={{ background:"#fff", borderRadius:12, overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,0.06)" }}>
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%", borderCollapse:"collapse", minWidth:780 }}>
            <thead><tr>{["Entreprise","Type","Poste","Statut","Date envoi","Date relance","Notes",""].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead>
            <tbody>
